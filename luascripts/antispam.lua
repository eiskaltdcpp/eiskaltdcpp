...
local nicks
local action
filterfile = GetConfigPath() .. "luascripts/Filter.txt"
settingsfile = GetConfigPath() .. "luascripts/Filtersettings.txt"
spamsfile = GetConfigPath() .. "luascripts/Filterspams.txt"
nicksfile = GetConfigPath() .. "luascripts/Filternicks.txt"
ignorelistfile = GetConfigPath() .. "DCPlusPlus.xml"
historyfile = GetConfigPath() .. "luascripts/Filterhistory.txt"
--reneuwing and (or) loading reneuwing the needed files
--megújítjuk és (vagy) betöltjük a spamgyûjtõ fájlt
...
--a favorites helyének alapján megnézzük, melyik kliens fut
...
--beállítások betöltése szettingsfájlból
...
function( hub, message, ret )
	...
	if params[1] == "/as" then
		...
		if params[2] == "readspam" then
			...
			spamnumber = params[3]
			local f = io.open(GetConfigPath() .. "luascripts/Filterspams.txt", "r")
			if f then
				local line = f:read()
...
function antispam.spamsfilewrite()
	...
	spamsfile = io.open(GetConfigPath() .. "luascripts/Filterspams.txt", "a+")
	if spamsfile then
		spamsfile:write("\nSPAM ")
