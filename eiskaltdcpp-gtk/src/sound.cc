...
#else
	FILE *pipe = popen((WulforSettingsManager::getInstance()->getString("sound-command") + " \"" +target+"\"" ).c_str(), "r" );
	//g_print("%s\n",(WulforSettingsManager::getInstance()->getString("sound-command") + " \"" +target+"\"" ).c_str());
	gchar *command_res;
